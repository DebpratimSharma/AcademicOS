...
import { Calendar as CalendarIcon, Trash2, Save, Loader2, CircleChevronRight } from "lucide-react"
import { createClient } from "@/utils/supabase/client"
import { format } from "date-fns"
import { toast } from "sonner" 
import { useRouter } from "next/navigation"

export function HolidayDrawer({ customTrigger }: { customTrigger?: React.ReactNode }) {
  ...
  return (
    <Drawer>
      ...
      <DrawerContent className="bg-background border-border md:mx-auto max-w-2xl pb-8 border text-foreground">
        <div className="mx-auto w-full max-w-sm">
          <DrawerHeader>
            <DrawerTitle className="text-center text-xl font-bold italic">Manage Holidays</DrawerTitle>
          </DrawerHeader>
          ...
          <div className="p-4 bg-card rounded-2xl border mb-4">
            <Calendar
              ...
              modifiersClassNames={{ holiday: "bg-destructive/20 text-destructive font-black rounded-md" }}
            />
          </div>
          ...
          <div className="flex flex-col px-5 gap-3">
            ...
            <DrawerClose asChild>
              <Button variant="ghost" className="text-muted-foreground hover:bg-muted rounded-xl">Cancel</Button>
            </DrawerClose>
          </div>
        </div>
      </DrawerContent>
